...
![Kandinsky in UI のショーケース: seedごとの装飾レイヤー、ブランドマーク、パレット差し替え、ダークモード対応](demo/showcase.svg)

この画像は手描きのモックではなく、`demo/build-showcase.mjs` が `dist/` の生成ロジックを
...
- **敷く面の背景色に近い色をpaletteに入れないこと。** 円は半透明+ブレンドで描くため、
  背景と近い色は「薄い形」ではなく消える（ショーケース用のパレットを選ぶ過程で、
  `#F2E8CF` を既定の紙色 `#F7F1E3` の上に置いたところ実際にほぼ見えなくなった）。
